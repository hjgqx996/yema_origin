...
static int ql_tzapp_read_key(void **ppbuf, uint32 *psize)
{
	char temp[56] = {0x7e,0x9d,0x67,0x1e,0x34,0x10,0x55,0xfd,0xf6,0x9d,
					 0x17,0xe7,0xb0,0x50,0xea,0x47,0x2d,0x47,0x4d,0xdd,
					 0xe3,0x70,0xd9,0x78,0xa6,0xed,0xec,0xe9,0x3d,0xdc,
					 0x39,0x9a,0x61,0xf1,0xd4,0x10,0x07,0xad,0xec,0x74,
					 0xf4,0x92,0x92,0x02,0xcc,0x3d,0xce,0x4a,0x56,0x7b,
					 0x2b,0xf7,0x0d,0x90,0xb7,0xec};
    *ppbuf = NULL;
    *psize = 56;
    *ppbuf = malloc(56);
	memcpy(*ppbuf,temp,56);
    return 1;
}
...
static int ql_tzapp_aes_gen_key(int tz_fd, const char *keyfile)
{
    ...
    if (keyblob.key_ptr) {
        ...
        Log(__FUNCTION__,"TZ_AES_GEN_KEY ret = %d, key_ptr = %p, key_size = %d\n",
...
static int ql_tzapp_aes_import_key(int tz_fd, const char *keyfile)
{
    ...
    if (ikeyblob.key_ptr) {
        ...
        Log(__FUNCTION__,"TZ_AES_IMPORT_KEY ret = %d, key_ptr = %p, key_size = %d\n",
...
int ql_tzapp_aes_encrypt_data(const char *infile, const char *outfile)
{
    int tz_fd = open("/dev/tzone", O_RDWR);
    if (tz_fd <= 0)
    {
    	Log(__FUNCTION__,"/dev/tzone open failed");
        return -1;
    }	
    int     ret;
    ...
    ql_tzapp_read_key(&keyblob.key_ptr, &keyblob.key_size);
    ...
    Log(__FUNCTION__,"TZ ENC DATA key_size = %d, input data size = %d\n",
            keyblob.key_size, input_data_size);
    ret = ioctl(tz_fd, QL_TZ_CRYPTO_SVC_AES_DATA_ENCRYPT, &enc_data);

    Log(__FUNCTION__,"TZ_AES_ENCRYPT_DATA ret = %d, output_data_size = %d\n",
...
int ql_tzapp_aes_decrypt_data(const char *infile, const char *outfile)
{
    int tz_fd = open("/dev/tzone", O_RDWR);
    if (tz_fd <= 0)
    {
    	Log(__FUNCTION__,"open tzone failed");
        return 0;
    }	
    int     ret = -1;
    uint32  input_data_size;
    ...
    Log(__FUNCTION__,"TZ_AES_DECRYPT_DATA ret = %d, output_data_size = %d\n",
...
static int ql_tzapp_rsa_generate_key(int tz_fd, const char *keyfile)
{
    ...
    if (rsa_key_blob_req.rsa_key_blob) {
        ...
        Log(__FUNCTION__,"TZ_AES_GENERATE_KEY ret = %d\n", ret);
...
static int ql_tzapp_rsa_import_keypair(int tz_fd, const char *outfile)
{
    ...
    Log(__FUNCTION__,"TZ_RSA_VERIFY ret = %d, mod_size = %d, pubexp_size=%d\n",
...
static int ql_tzapp_rsa_export_pubkey(int tz_fd, const char *keyfile, const char *pubkeyfile)
{
    ...
    Log(__FUNCTION__,"TZ_RSA_VERIFY ret = %d, modulus_size = %d, public_exponent_size=%d\n",
...
static int ql_tzapp_random_gen(int tz_fd, const char *keyfile) 
{
    ...
    if (random_data.data_ptr) {
        ...
        Log(__FUNCTION__,"TZ_AES_GENERATE_KEY ret = %d, data_ptr = %p, data_size = %d\n",
